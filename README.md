...
curl -X GET "$BASE/orders?page=0&size=20" \
  -H "Authorization: Bearer $USER_TOKEN"
...
curl -X POST "$BASE/orders/$ORDER_ID/cancel" \
  -H "Authorization: Bearer $USER_TOKEN"
```
...
```bash
# 1) Ошибка валидации
curl -X POST "$BASE/auth/login" -H 'Content-Type: application/json' -d '{"username":"a","password":"1"}'

# 2) Нет токена
curl -X POST "$BASE/orders" -H 'Content-Type: application/json' -d '{"items":[]}'
